...
void iter2(double * app_time, int *fib, int usec) {
    long t_start,t_end;
...
int main(int argc, char* argv[]) {
    ...
    if (me ==0) printf("\nApplication time = %f \n", app_time);
/*    nanos_event_value_t ev = 1;
    // Raising open event for iteration i
    nanos_instrument_begin_burst_with_val ( "Filler calculus", "Filler calculus()", &ev );

    int i;
    for(i = 0; i < 3000; i++) iter2(&iter_time, &fib, usec);

    // Raising close event for iteration i
    nanos_instrument_end_burst_with_val ( "Filler calculus", &ev );
    printf("%d\n", fib);*/
    MPI_Finalize();
    return EXIT_SUCCESS;
}
